##### Introduction #####
#This is an R script. We try to make our analysis clear and reproducible.
#We will structure it in subsections (#####)

##### Setup #####
#First we need to load the data and the required packages
#If everything goes well, the data should load automatically from our dropbox
#you will probably need to install some of the packages I use though
...
#TODO check whether we should remove the short pieces
...
##### Descriptives #####
#We can use some descriptive operations to answer easy questions about our data
...
### initial word count ###
token_abstracts <- data %>% #by unnesting the tokens we create so called unigrams meaning single words
...
#we can see that we remove 175 stopwords from the snowball library
